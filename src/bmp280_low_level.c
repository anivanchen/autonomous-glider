...
/*writes an array (data[]) of arbitrary size (dataLength) to I2C address (deviceAddress), starting from an internal
 * register address (startRegisterAddress)*/
...
/*reads an array (data[]) of arbitrary size (dataLength) from I2C address (deviceAddress), starting from an internal
 * register address (startRegisterAddress)*/
...
void delay_function(uint32_t delayMS) { sleep_ms(delayMS); }

/*implements a power function (used in altitude calculation)*/
float power_function(float x, float y) { return pow(x, y); }