...
static calibration_param_t
    dig1; /*dig1 and dig2 are based on calibration_param_t struct, which encapsulates all calibration values*/
static calibration_param_t dig2;
static int32_t t_fine; /*used for pressure compensation, changes with temperature*/

static void bmp280_set_bits_in_register(uint8_t deviceAddress, uint8_t registerAddress, uint8_t fieldData,
                                        uint8_t fieldStartBitAddress, uint8_t fieldLength);
...
static void bmp280_get_bits_in_register(uint8_t deviceAddress, uint8_t registerAddress, uint8_t *fieldData,
                                        uint8_t fieldStartBitAddress, uint8_t fieldLength);

/*initializer, detects the connected chips automatically and resets all of them. returns the detected addresses (if
 * any), and automatically gets the calibration data for further calculations. also sets the default values.
 */
i2c_address_t bmp280_init() {
  bmp280_i2c_init();

  i2c_address_t checkConnection =
      bmp280_check_connected_address(); /*automatically detects all connected bmp280 chips and their i2c address*/

  if (checkConnection ==
      BOTH_DETECTED) /*reseting, getting calibration data and setting default values in all connected chips*/
  {
...
/*one function to completely set up bmp280 mode, temperature and pressure oversampling, normal mode stadby time and IIR
 * filter coefficient*/
void bmp280_set(uint8_t deviceAddress, operation_mode_t operationMode, over_sampling_t tempOS,
                over_sampling_t pressureOS, standby_time_t standbyTime, iir_filter_t filterCoefficient) {
...
int32_t bmp280_get_temperature(uint8_t deviceAddress) {
  calibration_param_t *dig =
      bmp280_which_dig(deviceAddress); /*automatically relating device address and calibration data*/
  ...
  var2 =
      (((((adc_T >> 4) - ((int32_t)dig->T1)) * ((adc_T >> 4) - ((int32_t)dig->T1))) >> 12) * ((int32_t)dig->T3)) >> 14;
...
float bmp280_calculate_altitude_quick(uint8_t deviceAddress, uint32_t barometricPressure) {
  ...
  altitudeFloat =
      44307.69396 *
      (1 - 0.111555816 * power_function(barometricPressure, 0.190284)); /*calculating altitude from barometric formula*/

  return altitudeFloat;
}

/*calculates altitude from barometric pressure and temperature as arguments*/
float bmp280_calculate_altitude_hypsometric(uint8_t deviceAddress, uint32_t barometricPressure,
                                            float ambientTemperatureInC) {
  float altitudeFloat;

  altitudeFloat = ((ambientTemperatureInC + 273.15) *
                   (power_function((float)SEA_LEVEL_PRESSURE / (float)barometricPressure, (float)1 / 5.257) - 1)) /
                  0.0065;

  return altitudeFloat;
}

/*returns a complete set of sensor readings and altitude calculation (quick). if bmp280 is in sleep mode, gets data by
 * setting bmp280 to forced mode then back to sleep mode*/
