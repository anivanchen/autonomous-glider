...
void ICM_20948::debugPrintStatus(ICM_20948_Status_e stat) {
  switch (stat) {
    ...
    case ICM_20948_Stat_NoData:
      std::cout << "Data Underflow" << std::endl;
      break;
    case ICM_20948_Stat_SensorNotSupported:
      std::cout << "Sensor Not Supported" << std::endl;
      break;
    case ICM_20948_Stat_DMPNotSupported:
      std::cout << "DMP Firmware Not Supported. Is #define ICM_20948_USE_DMP "
                   "commented in util/ICM_20948_C.h?"
                << std::endl;
      break;
    case ICM_20948_Stat_DMPVerifyFail:
      std::cout << "DMP Firmware Verification Failed" << std::endl;
      break;
    case ICM_20948_Stat_FIFONoDataAvail:
      std::cout << "No FIFO Data Available" << std::endl;
      break;
    case ICM_20948_Stat_FIFOIncompleteData:
      std::cout << "DMP data in FIFO was incomplete" << std::endl;
      break;
    case ICM_20948_Stat_FIFOMoreDataAvail:
      std::cout << "More FIFO Data Available" << std::endl;
      break;
    case ICM_20948_Stat_UnrecognisedDMPHeader:
      std::cout << "Unrecognised DMP Header" << std::endl;
      break;
    case ICM_20948_Stat_UnrecognisedDMPHeader2:
      std::cout << "Unrecognised DMP Header2" << std::endl;
      break;
    case ICM_20948_Stat_InvalDMPRegister:
      std::cout << "Invalid DMP Register" << std::endl;
      break;
    default:
...
ICM_20948_Status_e ICM_20948::getBiasGyroX(int32_t *bias) {
  if (_device._dmp_firmware_available == true)  // Is DMP supported?
  {
    ...
    *bias = signedUnsigned32.signed32;  // Convert from unsigned to signed with
                                        // no cast ambiguity
    return status;
  }
  return ICM_20948_Stat_DMPNotSupported;
...
const char *ICM_20948::statusString(ICM_20948_Status_e stat) {
  ...
  switch (val) {
    ...
    case ICM_20948_Stat_SensorNotSupported:
      ...
      break;
    case ICM_20948_Stat_DMPNotSupported:
      return "DMP Firmware Not Supported. Is #define ICM_20948_USE_DMP "
             "commented in util/ICM_20948_C.h?";
      break;
    case ICM_20948_Stat_DMPVerifyFail:
...
ICM_20948_Status_e ICM_20948::checkID(void) {
  ...
  if (status != ICM_20948_Stat_Ok) {
    std::cout << "ICM_20948::checkID: ICM_20948_check_id returned: "
              << std::endl;
...
bool ICM_20948::isConnected(void) {
  ...
  std::cout << "ICM_20948::isConnected: checkID returned: " << std::endl;
  debugPrintStatus(status);
...
ICM_20948_Status_e ICM_20948::setSampleMode(uint8_t sensor_id_bm,
  ...
  sleep_ms(1000);  // Give the ICM20948 time to change the sample mode (see issue #8)
  return status;
}
...
ICM_20948_Status_e ICM_20948::clearInterrupts(void) {
  ...
  // todo: there may be additional interrupts that need to be cleared, like FIFO
  // overflow/watermark
...
ICM_20948_Status_e ICM_20948::WOMThreshold(uint8_t threshold) {
  ...
  status =
      ICM_20948_wom_threshold(&_device, &thr, &thr);  // write phase w/ readback
  if (status != ICM_20948_Stat_Ok) {
    return status;
  }
...
uint8_t ICM_20948::i2cMasterSingleR(uint8_t addr, uint8_t reg) {
  ...
  if (status != ICM_20948_Stat_Ok) {
    std::cout << "ICM_20948::i2cMasterSingleR: ICM_20948_i2c_master_single_r "
                 "returned: "
              << std::endl;
    debugPrintStatus(status);
  }
...
ICM_20948_Status_e ICM_20948::startupDefault(bool minimal) {
  ...
  if (retval != ICM_20948_Stat_Ok) {
    std::cout << "ICM_20948::startupDefault: checkID returned: " << std::endl;
  ...
  retval = swReset();
  if (retval != ICM_20948_Stat_Ok) {
    std::cout << "ICM_20948::startupDefault: swReset returned: " << std::endl;
    debugPrintStatus(retval);
    status = retval;
    return status;
  }
  ...
  retval = sleep(false);
  if (retval != ICM_20948_Stat_Ok) {
    std::cout << "ICM_20948::startupDefault: sleep returned: " << std::endl;
    debugPrintStatus(retval);
    status = retval;
    return status;
  }
  ...
  if (retval != ICM_20948_Stat_Ok) {
    std::cout << "ICM_20948::startupDefault: lowPower returned: " << std::endl;
    debugPrintStatus(retval);
    status = retval;
    return status;
  }
  ...
  if (retval != ICM_20948_Stat_Ok) {
    std::cout << "ICM_20948::startupDefault: startupMagnetometer returned: "
              << std::endl;
    debugPrintStatus(retval);
    status = retval;
    return status;
  ...
  {
    std::cout << "ICM_20948::startupDefault: minimal startup complete!"
              << std::endl;
    return status;
  }
  ...
  retval = setSampleMode(
      ...
      ICM_20948_Sample_Mode_Continuous);  // options:
                                          // ICM_20948_Sample_Mode_Continuous or
                                          // ICM_20948_Sample_Mode_Cycled
  if (retval != ICM_20948_Stat_Ok) {
    std::cout << "ICM_20948::startupDefault: setSampleMode returned: "
              << std::endl;
    debugPrintStatus(retval);
    status = retval;
    return status;
...
ICM_20948_Status_e ICM_20948::enableDMPSensor(enum inv_icm20948_sensor sensor,
      ...
      true)  // Should we attempt to enable the sensor?
  {
    status = inv_icm20948_enable_dmp_sensor(&_device, sensor,
                                            enable == true ? 1 : 0);
    std::cout << "ICM_20948::enableDMPSensor:  _enabled_Android_0: " << std::endl;
    std::cout << (int)_device._enabled_Android_0 << std::endl;
    std::cout << "  _enabled_Android_1: " << std::endl;
    std::cout << (int)_device._enabled_Android_1 << std::endl;
    std::cout << "  _dataOutCtl1: " << std::endl;
    std::cout << (int)_device._dataOutCtl1 << std::endl;
    std::cout << "  _dataOutCtl2: " << std::endl;
    std::cout << (int)_device._dataOutCtl2 << std::endl;
    std::cout << "  _dataRdyStatus: " << std::endl;
    std::cout << (int)_device._dataRdyStatus << std::endl;
    return status;
  }
  return ICM_20948_Stat_DMPNotSupported;
}
...
ICM_20948_Status_e ICM_20948::enableDMPSensorInt(
      ...
      true)  // Should we attempt to enable the sensor interrupt?
  {
    status = inv_icm20948_enable_dmp_sensor_int(&_device, sensor,
                                                enable == true ? 1 : 0);
    std::cout << "ICM_20948::enableDMPSensorInt:  _enabled_Android_intr_0: "
              << std::endl;
    std::cout << (int)_device._enabled_Android_intr_0 << std::endl;
    std::cout << "  _enabled_Android_intr_1: " << std::endl;
    std::cout << (int)_device._enabled_Android_intr_1 << std::endl;
    std::cout << "  _dataIntrCtl: " << std::endl;
    std::cout << (int)_device._dataIntrCtl << std::endl;
    return status;
  }
  return ICM_20948_Stat_DMPNotSupported;
}
...
ICM_20948_Status_e ICM_20948::setDMPODRrate(enum DMP_ODR_Registers odr_reg,
  ...
  {
    // In order to set an ODR for a given sensor data, write 2-byte value to DMP
    // using key defined above for a particular sensor. Setting value can be
    // calculated as follows: Value = (DMP running rate (225Hz) / ODR ) - 1 E.g.
...
ICM_20948_Status_e ICM_20948::setGyroSF(unsigned char div, int gyro_level) {
      ...
      true)  // Should we attempt to set the Gyro SF?
  {
    ...
    std::cout << "ICM_20948::setGyroSF:  pll: " << std::endl;
    std::cout << (int)_device._gyroSFpll << std::endl;
    std::cout << "  Gyro SF is: " << std::endl;
    std::cout << (int)_device._gyroSF << std::endl;
    return status;
  }
  return ICM_20948_Stat_DMPNotSupported;
}
...
ICM_20948_Status_e ICM_20948::initializeDMP(void) {
  ...
  if (_device._dmp_firmware_available != true) {
    std::cout
        << "ICM_20948::startupDMP: DMP is not available. Please check that you "
  ...
  // magnetometer to run at 100Hz by setting the CNTL2 register (0x31) to 0x08.
  // Then the ICM20948's I2C_SLV0 is configured to read nine bytes from the mag
  // every sample, starting from the STATUS1 register (0x10). ST1 includes the
  // DRDY (Data Ready) bit. Next are the six magnetometer readings (little
  // endian). After a dummy byte, the STATUS2 register (0x18) contains the HOFL
  // (Overflow) bit.
  //
  ...
  // running, the magnetometer is set to Single Measurement (SM) mode and that
  // ten bytes are read, starting from the reserved RSV2 register (0x03). The
  // datasheet does not define what registers 0x04 to 0x0C contain. There is
  // definitely some secret sauce in here... The magnetometer data appears to be
  // big endian (not little endian like the HX/Y/Z registers) and starts at
  // register 0x04. We had to examine the I2C traffic between the master and the
  // AK09916 on the AUX_DA and AUX_CL pins to discover this...
  //
  ...
  // unshifted) AK09916_REG_RSV2: we start reading here (0x03). Secret sauce...
  // 10: we read 10 bytes each cycle
  // true: set the I2C_SLV0_RNW ReadNotWrite bit so we read the 10 bytes (not
  // write them) true: set the I2C_SLV0_CTRL I2C_SLV0_EN bit to enable reading
  ...
  // I2C_SLV0_CTRL I2C_SLV0_GRP bit to show the register pairing starts at byte
  // 1+2 (copied from inv_icm20948_resume_akm) true: set the I2C_SLV0_CTRL
  // I2C_SLV0_BYTE_SW to byte-swap the data from the mag (copied from
  ...
  // 1: use I2C_SLV1
  // MAG_AK09916_I2C_ADDR: the I2C address of the AK09916 magnetometer (0x0C
  // unshifted) AK09916_REG_CNTL2: we start writing here (0x31) 1: not sure why,
  // but the write does not happen if this is set to zero false: clear the
  // I2C_SLV0_RNW ReadNotWrite bit so we write the dataOut byte true: set the
  // I2C_SLV0_CTRL I2C_SLV0_EN bit. Not sure why, but the write does not happen
  // if this is clear false: clear the I2C_SLV0_CTRL I2C_SLV0_REG_DIS (we want
  // to write the register value) false: clear the I2C_SLV0_CTRL I2C_SLV0_GRP
  // bit false: clear the I2C_SLV0_CTRL I2C_SLV0_BYTE_SW bit
  // AK09916_mode_single: tell I2C_SLV1 to write the Single Measurement command
  // each sample
  ...
  // effect. But it does. Maybe because the Gyro and Accel are placed in Low
  // Power Mode (cycled)? You can see by monitoring the Aux I2C pins that the
  // next three lines reduce the bus traffic (magnetometer reads) from 1125Hz to
  // the chosen rate: 68.75Hz in this case.
  result = setBank(3);
...
ICM_20948_Status_e ICM_20948_I2C::begin(struct i2c_wires wirePort, bool ad0val, uint8_t ad0pin)
{
  // Associate
  _ad0 = ad0pin;
  _i2c = wirePort;
  ...
  {
    gpio_init(_ad0);
    gpio_set_dir(_ad0, GPIO_OUT);
  }

  // Set pins to default positions
  if (_ad0 != ICM_20948_ARD_UNUSED_PIN)
  {
    gpio_put(_ad0, _ad0val);
  }
  ...
  if (status != ICM_20948_Stat_Ok)
  {
    std::cout << "ICM_20948_I2C::begin: startupDefault returned: " << std::endl;
    debugPrintStatus(status);
  }
  return status;
}
...
ICM_20948_Status_e ICM_20948::startupMagnetometer(bool minimal)
{
  ...
  {
    std::cout << "ICM_20948::startupMagnetometer: reached MAX_MAGNETOMETER_STARTS (";
    std::cout << (int)MAX_MAGNETOMETER_STARTS;
    std::cout << "). Returning ICM_20948_Stat_WrongID" << std::endl;
    status = ICM_20948_Stat_WrongID;
    return status;
  }
  else
  {
    std::cout << "ICM_20948::startupMagnetometer: successful magWhoIAm after ";
    std::cout << (int)tries;
  ...
  if (retval != ICM_20948_Stat_Ok)
  {
    std::cout << "ICM_20948::startupMagnetometer: writeMag returned: ";
    debugPrintStatus(retval);
    status = retval;
    return status;
  }
  ...
  if (retval != ICM_20948_Stat_Ok)
  {
    std::cout << "ICM_20948::startupMagnetometer: i2cMasterConfigurePeripheral returned: ";
    debugPrintStatus(retval);
    status = retval;
    return status;
  }
...
ICM_20948_Status_e ICM_20948::magWhoIAm(void)
{
  ...
  if (retval != ICM_20948_Stat_Ok)
  {
    std::cout << "ICM_20948::magWhoIAm: whoiam1: ";
    std::cout << (int)whoiam1;
    std::cout << " (should be 72) readMag set status to: ";
    debugPrintStatus(status);
    return retval;
  }
  ...
  if (retval != ICM_20948_Stat_Ok)
  {
    std::cout << "ICM_20948::magWhoIAm: whoiam1: ";
    std::cout << (int)whoiam1;
    std::cout << " (should be 72) whoiam2: ";
    std::cout << (int)whoiam2;
    std::cout << " (should be 9) readMag set status to: ";
    debugPrintStatus(status);
    return retval;
  }
  ...
  std::cout << " (should be 72) whoiam2: ";
  std::cout << (int)whoiam2;
  std::cout << " (should be 9). Returning ICM_20948_Stat_WrongID" << std::endl;
